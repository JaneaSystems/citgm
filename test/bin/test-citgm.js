...
test('bin: omg-i-fail /w custom script /w tap to file /w junit to file', function (t) {
  t.plan(1);
  var proc = callCitgm(['omg-i-fail', '-l', './test/fixtures/custom-lookup-script.json', '--tap', '/dev/null', '--junit', '/dev/null']);
  proc.on('error', function(err) {
    t.error(err);
