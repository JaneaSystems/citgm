...
test('bin: omg-i-have-script /w custom script /w tap to file /w junit to file  /w append', function (t) {
  t.plan(1);
  var proc = callCitgm(['omg-i-pass', '-l', './fixtures/custom-lookup-script.json', '--tap', '/dev/null', '--junit', '/dev/null', '--append']);
  proc.on('error', function(err) {
    t.error(err);
    t.fail('we should not get an error testing omg-i-pass');
  });
...
test('bin: citgm multiple test cases', function(t) {
  t.plan(1);
  var proc = callCitgm(['omg-i-pass', '-l', './fixtures/custom-lookup-multiple-tests.json']);
  proc.on('error', function(err) {
    t.error(err);
    t.fail('we should not get an error testing omg-i-pass');
  });
  proc.on('close', function (code) {
    t.equal(code, 0, 'omg-i-pass should pass');
  });
});
