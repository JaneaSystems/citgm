...
function parseTestCommand(testCommand) {
  ...
  if (testCommand[process.platform]) {
    return parseTestCommand(testCommand[process.platform]);
  }
  if (testCommand.command) {
    return {
      'command': testCommand.command,
      'type': testCommand.type || 'shell'
    };
  }
}
  ...
  if (meta) {
    ...
    if (rep) {
      ...
      if (rep.gitclone) {
        ...
        context.module.raw = normgit(rep.repo || meta.repository.url).url;
        context.emit('info','git-clone',context.module.raw);
      }      
      if (rep.install) {
        context.emit('info','lookup-install',rep.install);
        context.options.install = rep.install;
      }
      if (rep.script) {
      ...
      if (rep['test-command']) {
        const testCommand = parseTestCommand(rep['test-command']);
        if (testCommand) {
          context.options.testCommand = testCommand;
          context.emit('info','testCommand', testCommand.command);
          context.emit('info','testCommand-type', testCommand.type);
        }
      }
      if (rep.env) {
        context.emit('info','lookup-env',rep.env);
        context.options.env = rep.env;
      }
