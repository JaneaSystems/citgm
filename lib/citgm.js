...
Tester.prototype.run = function() {
  var self = this;
  const lookupTable = lookup.get(self.options);
  if (!lookupTable) {
    self.emit('fail','Lookup table could not be loaded');
    self.emit('end');
    return;
  }
  const moduleTests = lookupTable[this.module.name];
  const tests = moduleTests instanceof Array ? moduleTests : [moduleTests];

  async.forEachOfSeries(tests, function(test, index, next) {
    const runner = new TestRunner(Object.assign({}, self.module), test, Object.assign({}, self.options));
    self.runner = runner;

    runner.on('start', function(name) {
      self.emit('start', name, test['test-name']);
    }).on('fail', function(err) {
      self.emit('fail', err);
    }).on('data', function(type,key,message) {
      self.emit('data', type, key, message);
    }).on('done', function(result) {
      result.test = test['test-name'];
