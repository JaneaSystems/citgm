...
function Tester(mod, options) {
  ...
  const lookupTable = lookup.get(options);
  const moduleTests = lookupTable[this.module.name];
  this.tests = moduleTests instanceof Array ? moduleTests : [moduleTests];  
}
...
Tester.prototype.run = function() {
  var self = this;
  async.forEachOfSeries(self.tests,
    function(test, index, next) {
