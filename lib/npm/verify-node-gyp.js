...
function VerifyNodeGyp(context) {
  ...
  const nodeGypBinary = process.platform === 'win32' ?
    path.resolve(path.dirname(npmLocation), 'node_modules', 'npm', 'node_modules', 'node-gyp', 'bin', 'node-gyp.js') :
    path.resolve(path.dirname(npmLocation), '..', 'lib', 'node_modules', 'npm', 'node_modules', 'node-gyp', 'bin', 'node-gyp.js');
  ...
  fs.writeFileSync(this.script, script, {mode: 0o777});
