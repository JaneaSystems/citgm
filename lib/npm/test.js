...
function test(context, next) {
  ...
  if (context.options.testCommand) {
    const testCommand = context.options.testCommand;
    script.runTestCommand(nodeBin, npmBin, context, testCommand.command, testCommand.type, next);
    return;
  }
