...
function test(context, next) {
  ...
  if (context.options.testscript) {
    const testscript = typeof context.options.testscript === 'string' ? 
                       context.options.testscript :
                       context.options.testscript[process.platform]; 
    if (testscript) {
      script.testScript(nodeBin, npmBin, context, testscript, next);
      return;
    }
  }
